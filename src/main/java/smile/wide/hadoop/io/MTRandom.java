 ...
 *         <p>
 *         A Java implementation of the MT19937 (Mersenne Twister) pseudo random number generator
 ...
 *         <p>
 *         As a subclass of java.util.Random this class provides a single canonical method next()
 *         for generating bits in the pseudo random number sequence. Anyone using this class should
 *         invoke the public inherited methods (nextInt(), nextFloat etc.) to obtain values as
 *         normal. This class should provide a drop-in replacement for the standard implementation
 *         of java.util.Random with the additional advantage of having a far longer period and the
 *         ability to use a far larger seed value.
 *         <p>
 *         This is <b>not</b> a cryptographically strong source of randomness and should <b>not</b>
 ...
 *         href="http://creativecommons.org/licenses/LGPL/2.1/">CC-GNU LGPL</a>. <!-- /Creative
 *         Commons License -->
 * 
 ...
 * 
 *         <Work rdf:about=""> <license rdf:resource="http://creativecommons.org/licenses/LGPL/2.1/"
 ...
 *         </rdf:RDF> -->
 * 
 * @see <a href="http://www.math.sci.hiroshima-u.ac.jp/~m-mat/MT/emt.html" target="blank">Mersenne Twister Home Page</a>
 */
 ...
 * <p>
 * If the compatibility flag is set to true, then the algorithm will be seeded with the same default
 * value as was used in the original C code. Furthermore the setSeed() method, which must take a 64
 * bit long value, will be limited to using only the lower 32 bits of the seed to facilitate
 * seamless migration of existing C code into Java where identical behaviour is required.
 * <p>
 * Whilst useful for ensuring backwards compatibility, it is advised that this feature not be used
 * unless specifically required, due to the reduction in strength of the seed value.
 * 
 * @param compatible
...
/**
 * This version of the constructor simply initializes the class with the
 ...
 * identical.
 * <p>
 * If this instance was initialised in 'compatibility' mode then this method will only use the lower
 * 32 bits of any seed value passed in and will match the behaviour of the original C code exactly
 * with respect to state initialisation.
 * 
 * @param seed
 ...
 * <p>
 * If the length of the byte array is not a multiple of 4 then it is implicitly padded with zeros as
 ...
 * empty integer array, but the setSeed() method will throw an exception if the empty integer array
 * is passed to it.
 * 
 * @param buf
